#!/usr/bin/env python3
"""Render an editorial CSV as a GitHub-readable Swedish Markdown brief."""
...
REPORT_SITE_ORDER = (
    ("DN Ledare", "DN Ledare"),
    ("SvD Ledare", "SvD Ledare"),
    ("Di Ledare", "Di Ledare"),
    ("Expressen Ledare", "Expressen Ledare"),
    ("Aftonbladet Ledare", "Aftonbladet Ledare"),
    ("GP Ledare", "GP Ledare"),
    ("Sydsvenskan Huvudledare", "Sydsvenskan Huvudledare"),
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Render outputs/ledartexter/csv/editorials_YYYYMMDD.csv "
            "as a Swedish Markdown editorial brief."
        )
    )
    parser.add_argument(
        "--date",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "outputs/ledartexter/md/editorials_YYYYMMDD.md."
        ),
    )
...
def render_article_note(article: Article, heading_level: int = 4) -> list[str]:
    ...
    return [
        ...
        "",
        f"**Byline/redaktion:** {markdown_escape(article.authors or 'Ej angivet')}",
        "",
        f"**Länk:** <{article.link}>",
        "",
    ]
...
def render_report(articles: list[Article], target_date: date, timezone: ZoneInfo) -> str:
    ...
    lines.append("")
    lines.append(f"- **Texter:** {len(articles)}")
    ...
    lines.append("")
    lines.append("| Publikation | Texter |")
    lines.append("|---|---:|")
    ...
    lines.append("")
    lines.extend(render_queue_table(articles, target_date))
    lines.append("")
    ...
    for site in ordered_sites(set(by_site)):
        ...
        lines.append("")
        for article in newest_first(by_site[site]):
...
def main() -> int:
    ...
    input_path = Path(args.input or f"outputs/ledartexter/csv/editorials_{date_stamp}.csv")
    output_path = Path(args.output or f"outputs/ledartexter/md/editorials_{date_stamp}.md")
    ...
    print(f"Wrote {len(articles)} editorial texts to {output_path}")
    return 0
